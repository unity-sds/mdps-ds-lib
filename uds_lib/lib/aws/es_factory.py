from uds_lib.lib.utils.factory_abstract import FactoryAbstract
...
class ESFactory(FactoryAbstract):
    ...
    def get_instance(self, class_type, **kwargs):
        ct = class_type.upper()
        if ct == self.NO_AUTH:
            from uds_lib.lib.aws.es_middleware import ESMiddleware
            return ESMiddleware(kwargs['index'], kwargs['base_url'], port=kwargs['port'])
        if ct == self.AWS:
            from uds_lib.lib.aws.es_middleware_aws import EsMiddlewareAws
